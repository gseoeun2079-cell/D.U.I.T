...
if menu == "📅 학사 일정 & 급식":
    ...
    search_date = pd.to_datetime(st.date_input("날짜 선택"))
...
# =========================
    ...
    selected_date = datetime.combine(st.date_input("기준 날짜 선택"), datetime.min.time())

    # ➕ 추가 (form 사용 → 오류 방지 + 자동 초기화)
    with st.expander("➕ 수행평가 추가"):
    ...
    if not hw_tasks:
        ...
        hw_tasks.sort(key=lambda x: (x["done"], x["deadline"]))
        ...
        for i, t in enumerate(hw_tasks):
            ...
            with col1:
                ...
                if t["done"]:
                    ...
                    st.write(f"{t['task']} (D-{d_day})")

            with col2:
                if st.button("완료", key=f"hw_done_{i}"):
                    t["done"] = True
                    save_data(tasks)
                    st.rerun()
